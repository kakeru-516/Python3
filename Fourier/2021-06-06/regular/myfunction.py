...
def fft(data, samplingFreq, Fs, overlaprate):
  time_array, N_ave = overlap(data, samplingFreq, Fs, overlaprate)
  time_array, acf = boxcar(time_array, Fs, N_ave)
  fft_array, fft_mean, fft_axis = fft_ave(time_array, samplingFreq, Fs, N_ave, acf)
  id_maxOfAmp = fft_mean.argmax()
  if fft_axis[id_maxOfAmp] >= 4 and fft_axis[id_maxOfAmp] <= 5:
    return 255
  else:
    return 0
